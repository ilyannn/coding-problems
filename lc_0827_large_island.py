...
class Solution:
    ...
    def largestIsland(self, grid: List[List[int]]) -> int:
        ...
        def neighbors(x, y):
            return set(n for n in [(x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)] if all(c in R for c in n))
        ...
        return max(list(isles.values()) +
                   [sum(isles[isle] for isle in set(g[n] for n in neighbors(*k)) if isle) + 1
                    for k, v in g.items()
                    if not v]
                   )
